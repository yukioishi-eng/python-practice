#ドメイン層とアプリケーション層の区別
    #ドメイン層がやることは支払いや在庫管理などのクラス単体でできること
    #アプリケーション層がやることは商品の購入(支払い、在庫減少、商品状態の遷移)などの複数のAggregateをまたぐもの
#ドメイン層
...
class Order:
    ...
    #pay()を職務分離
    def mark_as_paid(self, amount: int):
...
class PayOrderUseCase:
    ...
    def execute(self, order_id: OrderId):
        ...
        price = product.price
        if order._coupon:
            price = order._coupon.apply(price)
